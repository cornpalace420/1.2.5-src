...
public class FSImpl {
	...
	public static final OpenState openDaFileSystem(String name) {
		DatabaseOpen dbo = AsyncHandlers.openDB(name);
		if(dbo == null) {
			err = "Unknown Error";
	...
	public static final boolean mkdir(String dir) {
	...
	public static final boolean renameFile(String oldPath, String newPath) {
		copyFile(oldPath, newPath);
		AsyncHandlers.deleteFile(db, oldPath);
		return !exists(oldPath) && exists(newPath);
	}
	...
	public static final boolean deleteFile(String path) {
		return AsyncHandlers.deleteFile(db, path).bool;
	}
