...
public class SpriteSheetTexture {
    ...
    public ByteBuffer grabFrame(int lvl) {
        int offWidth = 16 >> lvl;
    ...
    public void reloadData() {
        ...
        byte[] imgBytes = EaglerAdapterImpl2.loadResourceBytes("/sprite_sheet/" + name + ".png");
        if (imgBytes == null) {
            return;
        }

        int dim = iconTileSize * iconTileSize * 256;
        BufferedImage img = EaglerAdapterImpl2.loadPNG(imgBytes);
        dataFrameCount = img.getData().length / dim;

        ArrayList<Integer> loadedFrameSet = new ArrayList();
        byte[] metaBytes = EaglerAdapterImpl2.loadResourceBytes("/sprite_sheet/" + name + ".txt");
        ...
        int[] texData = img.getData();
        mipmapOffsets[0] = 0;
        putIntArrayToByteBuffer(buffer, texData);
    ...
    private static void putIntArrayToByteBuffer(ByteBuffer buffer, int[] array) {
        for (int value : array) {
            buffer.putInt(buffer.position() + 1, value);
        }
    }
